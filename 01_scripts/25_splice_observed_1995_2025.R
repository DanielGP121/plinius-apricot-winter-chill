...
# period on its own. Script 24 established that on the seasons they share the two agree to +0.13 CP
...
# check of script 24 (mean absolute error <= 3 CP), so the conclusion can be seen not to depend on
# where that line was drawn.
#
# Outputs: observed_spliced_swc.csv, observed_spliced_summary.csv and fig24_01.
#
# Usage:
#   Rscript 25_splice_observed_1995_2025.R
#   Rscript 25_splice_observed_1995_2025.R --min-recent 4 --max-mae 3
#
...
MAX_MAE    <- as.numeric(getarg("--max-mae", 3))     # station agreement threshold from script 24
...
# --- § 4 - with and without the stations flagged in script 24 ------------------------------------
if (file.exists(AGREE)) {
  ag <- fread(AGREE, colClasses = list(character = "station_id"))
  if (!"mae" %in% names(ag)) stop("the agreement table has no mae column; re-run script 24")
