...
# baseline can be nearly doubled (script 15 --windows obs --per-season --years 1975,2020) and the
...
# come from the PNACC archive and 2021-2025 from the AEMET API. Script 24 measured the API reading
...
#
# Usage:
#   Rscript 26_observed_long_record.R
#
...
API_OFFSET <- 0.13        # measured in script 24; quoted in the caption, not applied to the data
...
if (!file.exists(SPLICE)) stop(sprintf("missing splice table at %s (run script 25 first)", SPLICE))
