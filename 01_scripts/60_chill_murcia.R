...
# report 61_chill_maps_murcia.Rmd, which sources this file so there is a single source of truth.
#
...
#
# Input: the chill-format slice from 12_pnacc_to_tables.py --chill-format, i.e. one CSV per
# scenario with columns scenario, model, station_id, lon, lat, Year, Month, Day, Tmax, Tmin.
#
# Usage (headless):
#   Rscript 60_chill_murcia.R <slice_dir> <out_csv> [max_stations] [start_jday] [end_jday]
# Usage (from R / Rmd):
#   source("60_chill_murcia.R"); res <- compute_chill_table(slice_dir)
#
...
main <- function() {
  ...
  if (length(args) < 2) {
    stop("uso: Rscript 60_chill_murcia.R <slice_dir> <out_csv> [max_stations] [start_jday] [end_jday]")
  }
  slice_dir    <- args[[1]]
  out_csv      <- args[[2]]
