#!/usr/bin/env python3
"""Assemble the animation frames written by 50_scenario_frames.R into looping GIFs.
...
of those it builds one track per scenario over the eleven-model sheets of 54_model_figures.R, which
...
Usage: python 51_make_gifs.py [--scale 1.0] [--only sidebyside]
...
# are drawn by 54_model_figures.R rather than by 50_scenario_frames.R, so no manifest describes them
