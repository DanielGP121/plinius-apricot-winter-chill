...
g34 <- ggplot(rc, aes(temp, cp_day)) +
  ...
  annotate("text", x = opt$temp + 0.7, y = opt$cp_day, hjust = 0, vjust = 0.2, size = 4.6,
           ...
           label = sprintf("óptimo: %s CP/día a %s °C", n_es(opt$cp_day, 3), n_es(opt$temp, 0))) +
  annotate("text", x = -11, y = opt$cp_day * 0.86, hjust = 0, size = 4.2, colour = "grey30",
           label = sprintf("a 0 °C sólo se acumula\nel %s %% del óptimo", n_es(pct(0), 0))) +
  annotate("text", x = 17.5, y = opt$cp_day * 0.55, hjust = 0, size = 4.2, colour = "grey30",
           label = "por encima de 14 °C\nno se acumula nada") +
  ...
  scale_y_continuous(labels = function(x) n_es(x, 1)) +
  labs(title = ttl("El frío que cuenta tiene un óptimo, y no está en el punto de congelación"),
       subtitle = "Respuesta del modelo dinámico (Fishman et al. 1987) medida sobre series de temperatura constante",
       x = "Temperatura (°C)", y = "Chill portions acumuladas por día") +
  talk_theme
...
g35 <- ggplot(ss, aes(season_end_year, CP)) +
  ...
  annotate("label", x = min(ss$season_end_year), y = mn, hjust = 0, size = 4.3, colour = "grey25",
           fill = "white", label = sprintf("media: %s CP", n_es(mn))) +
  annotate("label", x = min(ss$season_end_year), y = p10, hjust = 0, size = 4.3, colour = "#d7191c",
           fill = "white", fontface = "bold",
           label = sprintf("Safe Winter Chill (P10): %s CP", n_es(p10))) +
  labs(title = ttl("Al agricultor no lo arruina el invierno medio, sino el invierno pobre"),
       subtitle = sprintf("Estación %s · %d temporadas observadas · el P10 es el frío que se supera nueve de cada diez inviernos",
                          pick, nrow(ss)),
       x = NULL, y = "Chill portions de la temporada") +
  talk_theme
...
p1 <- ggplot() + geom_point(data = pts_p, aes(x, y, colour = SWC), size = 0.32) +
  scale_colour_viridis_c() + base_map("1. Lo que hay: estaciones",
                                      sprintf("%d puntos con frío calculado", nrow(pts)))
sdf <- as.data.frame(surf, xy = TRUE, na.rm = TRUE); names(sdf)[3] <- "SWC"
p2 <- ggplot() + geom_raster(data = sdf, aes(x, y, fill = SWC)) +
  scale_fill_viridis_c() + base_map("2. Interpolación IDW, radio 50 km",
                                    "celdas sin estación a menos de 50 km quedan vacías")
...
p3 <- ggplot() +
  ...
  base_map("3. Sólo donde hay cultivo, y en qué proporción",
           sprintf("%s km² de CORINE 211-244, sin pastos",
                   formatC(round(global(cropfrac, "sum", na.rm = TRUE)[1, 1] * cell_area_km2(cropfrac)),
                           format = "d", big.mark = ".")))

g36 <- (p1 | p2 | p3) +
  plot_annotation(
    title = ttl("Una red de estaciones no es un mapa, y un recuento de estaciones no es una superficie"),
    subtitle = "Interpolación y máscara replicando Egea et al. 2022 (Front. Plant Sci. 13:842628), con 3460 estaciones frente a las 270 de la referencia",
    theme = theme(plot.title = element_text(face = "bold", size = 17),
                  plot.subtitle = element_text(size = 12, colour = "grey30")))
ggsave(fig_path("fig36_method_chain.png"), g36, width = 15, height = 5.2 * MAP_AR + 1.5, dpi = 190,
       bg = "white")
...
g37 <- ggplot() + geom_raster(data = cdf, aes(x, y, fill = clase)) +
  ...
  scale_fill_manual(values = setNames(c("#2c7bb6", "#fdae61", "#d7191c"), LAB), drop = FALSE,
                    name = NULL) +
  labs(title = ttl("Hoy 'Búlida' cumple su requerimiento en casi todo el suelo cultivable de España"),
       subtitle = sprintf("Línea base 1995-2020 · solo el mutante sería viable en %s km² (%s %%), y ninguna de las dos en %s km²",
                          formatC(round(km2_base[2]), format = "d", big.mark = "."),
                          n_es(100 * km2_base[2] / sum(km2_base)),
                          formatC(round(km2_base[3]), format = "d", big.mark = "."))) +
  talk_theme +
  theme(panel.grid = element_blank(), axis.text = element_blank(), axis.title = element_blank())
ggsave(fig_path("fig37_baseline_today.png"), g37, width = 9.5, height = 9.5 * MAP_AR + 1.7,
       dpi = 190, bg = "white")
