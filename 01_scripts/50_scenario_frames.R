...
#
# Same chain as 30_cropland_viability_national.R (IDW at 50 km, CORINE cropland mask, three
...
# 51_make_gifs.py which frames make up which animation and how long each one is held.
#
# Usage: Rscript 50_scenario_frames.R [--res 1000] [--quick]
...
# Identical to 30_cropland_viability_national.R; the area statistics printed on the frames have to
...
hatch_for <- function(sit) {
  ...
  # The agreement layers are written at the resolution 32_per_model_stats.R ran at, normally 1 km.
  ...
  v <- if (!file.exists(f)) {
    message("no agreement layer for ", sit, "; run 32_per_model_stats.R")
    list()
  } else {
    ag <- rast(f)
...
cat("next: python 51_make_gifs.py\n")
