...
cat(sprintf("   THREDDS %s · archivo %s · temporadas descartadas %s de %s (%.2f %%)\n",
            i_es(N_THREDDS), i_es(N_FORM), i_es(SEAS_LOST), i_es(SEAS_NOMINAL),
...
if (!file.exists(cropfrac_file))
  stop("falta ", basename(cropfrac_file), "; ejecuta antes 36_per_model_stats.R", call. = FALSE)
...
cat(sprintf("   Espana %s · cultivable %s · alcanzado %s · en la tabla %s km2\n",
            i_es(AREA_SPAIN), i_es(AREA_CROP), i_es(AREA_REACHED), i_es(AREA_TABLE)))
...
if (abs(AREA_REACHED - AREA_TABLE) > 1)
  stop(sprintf(paste("la superficie que alcanza la interpolacion (%.1f km2) no coincide con la de",
                     "talk_numbers_cropland.csv (%.1f km2). Reejecuta 19_cropland_viability_national.R"),
...
step_panel <- function(steps, title, subtitle, unit, accent = "#2c7bb6") {
  ...
  ggplot(steps) +
    ...
    geom_text(aes(w + 0.025, y, label = sprintf("%s%s", i_es(value),
    ...
    geom_text(aes(1.42, y - 0.16, label = note), hjust = 0, size = 3.0, colour = "grey42",
              lineheight = 0.95) +
    scale_fill_manual(values = c(paso = accent, sale = "grey70"), guide = "none") +
...
est <- data.table(
  label = c("Servidas por el THREDDS", "Entran en el cálculo", "Archivo observado",
            "Cubiertas también por la API"),
  value = c(N_THREDDS, N_THREDDS, N_FORM, API_ST),
  kind  = c("paso", "paso", "paso", "paso"),
  note  = c(sprintf("el formulario web del mismo portal sirve %s:\nes una bifurcación, no un descarte",
                    i_es(N_FORM)),
            "ninguna se pierde por calidad",
            "1975-2020, la base del contraste",
            if (is.na(API_ST)) "" else
              sprintf("extiende a 2025 · %s de sus %s temporadas\ncaen por completitud < 85 %%",
                      i_es(API_ALL - API_KEPT), i_es(API_ALL))))

tem <- data.table(
  label = c("Posibles en las siete ventanas", "Superan el 85 % de completitud", "Descartadas"),
  value = c(SEAS_NOMINAL, SEAS_NOMINAL - SEAS_LOST, SEAS_LOST),
  kind  = c("paso", "paso", "sale"),
  note  = c("estación × modelo × temporada",
            sprintf("el %s %% de las posibles", n_es(100 * (1 - SEAS_LOST / SEAS_NOMINAL), 2)),
            sprintf("el %s %%, todas en %s;\nlas otras seis ventanas no pierden ninguna",
                    n_es(100 * SEAS_LOST / SEAS_NOMINAL, 2), seas[1]$periodo)))

sup <- data.table(
  label = c("España peninsular y Baleares", "Suelo cultivable CORINE",
            "Alcanzado por la interpolación", "Clasificado en las tres clases"),
  value = c(AREA_SPAIN, AREA_CROP, AREA_REACHED, AREA_TABLE),
  kind  = rep("paso", 4),
  note  = c("contorno de las CCAA, sin Canarias",
            sprintf("clases 211-244 sin pastos: el %s %% del país",
                    n_es(100 * AREA_CROP / AREA_SPAIN, 1)),
            sprintf("celdas con estación a menos de 50 km:\nse pierden %s km²",
                    i_es(AREA_CROP - AREA_REACHED)),
            "este es el denominador de toda cifra\nde superficie del trabajo"))

pa <- step_panel(est, "Las estaciones", "Ninguna se descarta en la rama de proyecciones.", "")
pt <- step_panel(tem, "Las temporadas",
                 "Aquí sí actúa el filtro de completitud, y muy poco.", "", accent = "#7a6ea8")
pb <- step_panel(sup, "El territorio",
                 sprintf("Celda de %s km², no de 1 km² exacto (ver 00_corine.R).", n_es(CELL, 5)),
                 "km²", accent = "#2e8b57")

extra <- if (!is.null(colo) && !is.null(cv) && nrow(cv))
  sprintf(paste("Dos cosas más que conviene saber de esas %s estaciones: %s de ellas comparten coordenada con otra (%s parejas),",
                "y %s no tiene ninguna vecina dentro del radio de 50 km."),
          i_es(N_THREDDS), i_es(sum(colo$n)), i_es(nrow(colo)), i_es(cv$n_no_neigh)) else ""

g51 <- ((pa / pt) | pb) +
  plot_layout(widths = c(1, 1)) +
  plot_annotation(
    title = ttl("De dónde sale cada estación y cada kilómetro cuadrado"),
    subtitle = paste("Recuentos reales en cada paso, sin porcentajes redondeados, para que la resta se pueda hacer a mano.",
                     "Cada panel tiene su propia unidad y su propia escala.\n", extra),
...
fwrite(rbind(
  data.table(lado = "estaciones", paso = est$label, valor = est$value),
  data.table(lado = "superficie", paso = sup$label, valor = sup$value),
  data.table(lado = "superficie", paso = "area de celda km2", valor = CELL)),
  out_path("attrition_funnel_numbers.csv"))
fwrite(seas, out_path("season_attrition_by_window.csv"))

cat(sprintf("\nescritas fig51, attrition_funnel_numbers.csv y season_attrition_by_window.csv\n"))
