...
#
# Usage: Rscript 57_attrition_funnel.R [--res 1000]
...
if (!file.exists(cropfrac_file))
  stop("missing ", basename(cropfrac_file), "; run 32_per_model_stats.R first", call. = FALSE)
...
if (abs(AREA_REACHED - AREA_TABLE) > 1)
  stop(sprintf(paste("the cropland the interpolation reaches (%.1f km2) does not match the one in",
                     "talk_numbers_cropland.csv (%.1f km2). Re-run 30_cropland_viability_national.R"),
