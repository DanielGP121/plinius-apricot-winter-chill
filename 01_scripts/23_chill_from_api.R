...
#
# Companion to 15_chill_national_parallel.R --per-season. That one reads the PNACC archive NetCDFs
# and stops in 2020; this one reads the CSV downloaded from the API (script 21), which reaches 2025
# but covers only 666 of the 3044 archive stations and is thin before 2008. Both emit the same table
# so 24_observed_api_vs_archive.R can join them season by season and decide whether the two observed
...
#
# Input : observed_1995_2025.csv[.gz] with station_id,Year,Month,Day,Tmax,Tmin (script 21 --merge)
# Output: chill_api_seasons.csv, same columns as chill_obs_seasons.csv
#
# Usage:
#   Rscript 23_chill_from_api.R --maxst 12                       # smoke test, own output path
#   Rscript 23_chill_from_api.R --cores 8
#   Rscript 23_chill_from_api.R --api <csv.gz> --coords <csv> --out <csv> --cores 8
#
...
# A smoke test writes its own file for the same reason as in script 15: a 12-station table is
# indistinguishable from a complete one once it is on disk, and script 24 would happily join it.
...
# These must stay identical to 15_chill_national_parallel.R. The comparison in script 24 is only
...
END_JDAY    <- 59L       # 28 Feb
MAX_NA_FRAC <- 0.4
MIN_PERC    <- 85        # not applied here; reported so script 24 can filter both sides identically
...
read_coords <- function(path) {
  if (!file.exists(path)) stop(sprintf("no archive season table at %s (run script 15 --per-season first)", path))
...
# Mirrors the --per-season branch of script 15 on purpose: same window, same two models, and the
